...
var deleteSession = async (ctx, id, move = moveToOsTrash) => {
  ...
  if (header === void 0) return { kind: "missing" };
  const location = ctx.sessionPersistence.locate?.(header);
  if (location === void 0) return { kind: "unsupported" };
...
var removeFromArchive = async (ctx, id) => {
  ...
  if (domain?.global === void 0) return;
  ...
  if (registry.enqueueOperation === void 0) {
    await unarchive();
    return;
  }
...
var safeList = async (ctx) => (await ctx.sessionPersistence.list()).map((snapshot) => snapshot.header);
